...
const methodDescriptions = {
  en: {
    ...
    uploadLogFile: 'Upload the current SDK log file and return the native upload request ID. Completion is reported by the <a href="#event-uploadLogResult">uploadLogResult</a> event.',
    ...
    setAudioProfile: 'Set audio profile and scenario. Prefer AgoraAudioProfile and AgoraAudioScenario instead of raw numbers.',
  ...
  zh: {
    ...
    uploadLogFile: '上传当前 SDK 日志文件，返回原生上传请求 ID。上传最终结果通过 <a href="#event-uploadLogResult">uploadLogResult</a> 事件通知。',
    ...
    setAudioProfile: '设置音频 profile 和 scenario。建议使用 AgoraAudioProfile 和 AgoraAudioScenario，避免直接写数字。',
...
const eventDescriptions = {
  en: {
    ...
    uploadLogResult: 'Fires when a log upload finishes, carrying the request ID, success flag, and failure reason.',
  ...
  zh: {
    ...
    uploadLogResult: '日志上传结束时触发，携带请求 ID、是否成功以及失败原因。',
...
function describeParam(locale, name, methodName) {
  const overrides = {
    en: {
      ...
      profile: {
        setAudioProfile: 'required audio profile. Prefer <code>AgoraAudioProfile</code> values.',
        setChannelProfile: 'required channel profile.',
        default: 'required profile value.',
      },
      role: 'required client role.',
      scenario: 'optional audio scenario. Prefer <code>AgoraAudioScenario</code> values.',
    ...
    zh: {
      ...
      path: '必填，本地文件路径。',
      profile: {
        setAudioProfile: '必填，音频 profile，建议使用 <code>AgoraAudioProfile</code>。',
        setChannelProfile: '必填，频道 profile。',
        default: '必填，profile 值。',
      },
      role: '必填，角色值。',
      scenario: '可选，音频 scenario，建议使用 <code>AgoraAudioScenario</code>。',
      interval: '必填，毫秒级间隔。',
      smooth: '可选，平滑系数。',
      reportVad: '可选，语音活动标志。',
      backend: '必填，后端值。',
...
function describeEventField(locale, eventName, field) {
  ...
  const details = {
    en: {
      ...
      'txBytes?': 'optional total transmitted bytes.',
      'rxBytes?': 'optional total received bytes.',
      'txKBitRate?': 'optional total transmit bitrate in Kbps.',
      'rxKBitRate?': 'optional total receive bitrate in Kbps.',
      'txAudioBytes?': 'optional transmitted audio bytes.',
      'rxAudioBytes?': 'optional received audio bytes.',
      'txVideoBytes?': 'optional transmitted video bytes.',
      'rxVideoBytes?': 'optional received video bytes.',
      'txAudioKBitRate?': 'optional transmit audio bitrate in Kbps.',
      'rxAudioKBitRate?': 'optional receive audio bitrate in Kbps.',
      'txVideoKBitRate?': 'optional transmit video bitrate in Kbps.',
      'rxVideoKBitRate?': 'optional receive video bitrate in Kbps.',
      'lastmileDelay?': 'optional last-mile network delay.',
      'cpuTotalUsage?': 'optional total CPU usage ratio.',
      'gatewayRtt?': 'optional gateway round-trip time.',
      'cpuAppUsage?': 'optional app CPU usage ratio.',
      'users?': 'optional user count in the stats sample.',
      'connectTimeMs?': 'optional connection establishment time in milliseconds.',
      'txPacketLossRate?': 'optional transmit packet loss rate.',
      'rxPacketLossRate?': 'optional receive packet loss rate.',
      'memoryAppUsageRatio?': 'optional app memory usage ratio.',
      'memoryTotalUsageRatio?': 'optional total memory usage ratio.',
      'memoryAppUsageInKbytes?': 'optional app memory usage in KB.',
    ...
    zh: {
      ...
      'txBytes?': '可选，总发送字节数。',
      'rxBytes?': '可选，总接收字节数。',
      'txKBitRate?': '可选，总发送码率，单位 Kbps。',
      'rxKBitRate?': '可选，总接收码率，单位 Kbps。',
      'txAudioBytes?': '可选，发送音频字节数。',
      'rxAudioBytes?': '可选，接收音频字节数。',
      'txVideoBytes?': '可选，发送视频字节数。',
      'rxVideoBytes?': '可选，接收视频字节数。',
      'txAudioKBitRate?': '可选，发送音频码率，单位 Kbps。',
      'rxAudioKBitRate?': '可选，接收音频码率，单位 Kbps。',
      'txVideoKBitRate?': '可选，发送视频码率，单位 Kbps。',
      'rxVideoKBitRate?': '可选，接收视频码率，单位 Kbps。',
      'lastmileDelay?': '可选，最后一公里网络时延。',
      'cpuTotalUsage?': '可选，总 CPU 使用率。',
      'gatewayRtt?': '可选，网关往返时延。',
      'cpuAppUsage?': '可选，应用 CPU 使用率。',
      'users?': '可选，当前统计样本中的用户数。',
      'connectTimeMs?': '可选，建连耗时，单位毫秒。',
      'txPacketLossRate?': '可选，发送丢包率。',
      'rxPacketLossRate?': '可选，接收丢包率。',
      'memoryAppUsageRatio?': '可选，应用内存使用率。',
      'memoryTotalUsageRatio?': '可选，总内存使用率。',
      'memoryAppUsageInKbytes?': '可选，应用内存占用，单位 KB。',
      'reason?': '可选，离线原因码。',
  ...
  return details[locale][fieldName] ?? (locale === 'en' ? 'See the event contract for runtime semantics.' : '请结合事件契约理解运行时语义。');
}
