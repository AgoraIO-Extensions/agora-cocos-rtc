...
test('cocos integration scripts build and launch android and ios test apps', async () => {
  ...
  assert.match(androidScript, /REPORT_RAW_PATH="\$ROOT_DIR\/test_shard\/integration_test_app\/reports\/android-api-report\.raw\.json"/);
  ...
  assert.match(androidScript, /node "\$ROOT_DIR\/scripts\/collect-cocos-test-report\.mjs" android "\$REPORT_RAW_PATH"/);
